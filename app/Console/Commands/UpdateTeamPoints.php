...
class UpdateTeamPoints extends Command
{
    // Define the command signature and description
    protected $signature = 'update:team-points';
    ...
    public function handle()
    {
        ...
        foreach ($events as $event) {
            ...
            foreach ($teams as $team) {
                ...
                // }
                foreach ($teamPlayers as $teamPlayer) {
                    $matchPlayer = MatchPlayer::where('id', $teamPlayer->match_player_id)->first();

                    if ($matchPlayer) {
                        $pointsToAdd = $matchPlayer->points ?? 0;

                        // Check if the match player is the team's captain
                        if ($team->captain_match_player_id === $matchPlayer->id) {
                            $this->info("Doubling points for Captain Player ID: {$matchPlayer->id}");
                            $pointsToAdd *= 2; // Double the points for the captain
                        }
